...
import {updateStudio} from '../utils/requests/studio.ts'
import {useTranslation} from 'react-i18next'
import {getStudio} from "../utils/requests/studios.ts";
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    try {
        ...
        const studio = await updateStudio({...body, token })
        return redirect(`/pro/studios/edit/${studio.id}`)
...
export default function () {
    ...
    const { studio, errors, success } = useLoaderData<typeof loader>()
        ...
        <Form method='POST' className='w-full gap-4' encType='multipart/form-data'>
            ...
            <div className='flex w-full gap-8'>
                ...
                <div className='flex flex-col w-1/2 gap-4'>
                    ...
                    {/* NAME */}
                    <input placeholder={t('studio-name')} type="text" name='name' value={studio.name} className='outline-none bg-opacity-30 backdrop-blur-lg bg-black px-2 py-1 text-base rounded-md border-1 border-gray-700 focus:border-red-400 duration-300' />
                    {/* /NAME */}

                    {/* DESCRIPTION */}
                    <input placeholder={t('studio-description')} type="textarea" name='description' value={studio.description} className='outline-none bg-opacity-30 backdrop-blur-lg bg-black px-2 py-1 text-base rounded-md border-1 border-gray-700 focus:border-red-400 duration-300' />
                    {/* /DESCRIPTION */}

                    {/* MAX CAPACITY */}
                    <input placeholder={t('max-nb-employees')} type="number" name='maxCapacity' value={studio.maxCapacity} className='outline-none bg-opacity-30 backdrop-blur-lg bg-black px-2 py-1 text-base rounded-md border-1 border-gray-700 focus:border-red-400 duration-300' />
                    {/* /MAX CAPACITY */}
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                        <div className='w-full flex flex-col gap-1'>
                            ...
                            <input placeholder='Starting Date' type="time" name='openingTime' value={studio.openingTime} className='outline-none bg-opacity-30 backdrop-blur-lg bg-black px-2 py-1 text-base rounded-md border-1 border-gray-700 focus:border-red-400 duration-300' />
                        </div>
                        <div className='w-full flex flex-col gap-1'>
                            <span className='text-sm'>{t('closing-time')}</span>
                            <input placeholder='Starting Date' type="time" name='closingTime' value={studio.closingTime} className='outline-none bg-opacity-30 backdrop-blur-lg bg-black px-2 py-1 text-base rounded-md border-1 border-gray-700 focus:border-red-400 duration-300' />
                        </div>
                    </div>
                    {/* /OPENING TIMES */}
                </div>
            ...
            <div className='flex items-center gap-2 flex-wrap'>
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='monday' checked={studio.monday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='tuesday' checked={studio.tuesday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='wednesday' checked={studio.wednesday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='thursday' checked={studio.thursday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='friday' checked={studio.friday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='saturday' checked={studio.saturday=='on'} asChild>
                ...
                <Switch.Root className="Toggle" aria-label="Toggle italic" name='sunday' checked={studio.sunday=='on'} asChild>
            ...
            <div className="flex mt-4">
                <button className='px-4 py-2 bg-gray-700 rounded-lg text-white ml-auto'>{t('update')}</button>
            </div>
        </Form>
    </div>
}
